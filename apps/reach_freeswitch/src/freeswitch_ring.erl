...
hangup(Pid) ->
	lager:debug("Hanging channel ~p", [Pid]),
...
init(Options) ->
	lager:info("Starting ring channel", []),
	Module = proplists:get_value(module, Options),
	%%lager:debug("Freeswitch ring options ~p", [Options]),
	Callbacks = #callbacks{
	...
	AgentRec = proplists:get_value(agent, Options),
	RingTimeout = freeswitch_util:get_ring_timeout(AgentRec#agent.login),
	lager:info("Ringtimeout in freeswitch ring ~p", [RingTimeout]),

	Callrec = proplists:get_value(call, Options),
	Fsnode = case gproc:lookup_values({p, g, {freeswitch_node, Callrec#call.id}}) of
		[{_, N}] ->
			lager:info("freeswitch_node set in gproc as ~p for callId ~p", [N, Callrec#call.id]),
			N;
		_ ->
			Node = proplists:get_value(freeswitch_node, Options),
			lager:info("freeswitch_node set in options ~p for callId ~p", [Node, Callrec#call.id]),
			Node
	end,
	...
	case freeswitch:api(Fsnode, create_uuid) of
		{ok, UUID} ->
            ...
            DialVars = proplists:get_value(dial_vars, Options, DefaultDailOpts),
			PreInitDialStringOpts = [
					lists:flatten(["origination_caller_id_name='", CallerName,"'"]),
					...
					lists:flatten(["sip_h_X-DNIS='", Dnis, "'"])] ++ DialVars,
						...
						{ok, _BgApiId} ->
							...
							case Gethandle(Gethandle, 0) of
								{error, badsession} ->
									...
									{stop, normal};
								{error, Other} ->
									lager:error("other error starting; ~p for ~p", [Other, proplists:get_value(agent, Options)]),
									{stop, normal};
								_Else ->
...
handle_info({call, {event, [UUID | _Rest]}}, #state{cnode = Cnode, options = Options, uuid = UUID} = State) ->
	lager:info("ring call ~p", [UUID]),
	OptEvents = lists:sort(proplists:get_value(events, Options, [])),
	BaseEvents = lists:sort(['CHANNEL_ANSWER', 'CHANNEL_BRIDGE', 'CHANNEL_UNBRIDGE', 'CHANNEL_CALLSTATE', 'CHANNEL_HANGUP', 'PLAYBACK_STOP']),
...
handle_info({call_event, {event, [UUID | Rest]}}, #state{options = _Options, uuid = UUID, callbacks = #callbacks{handle_event = CbHandleEvent} = Callbacks} = State) ->
	...
	lager:info("ring call event ~p: ~p", [UUID, Event]),
...
terminate(Reason, #state{callbacks = #callbacks{terminate = Fun} = Callbacks} = State) ->
	...
	freeswitch:api(State#state.cnode, uuid_kill, State#state.uuid),
