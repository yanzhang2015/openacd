...
-export([
	...
	do_dial_string/3,
	do_dial_string_without_sip_opts/3,
...
%% </ul>
-spec(start/2 :: (Nodes :: list(atom()), Options :: start_opts()) -> {'ok', pid()}).
start(Nodes, [Head | _Tail] = Options) when is_tuple(Head) ->
	gen_server:start({local, ?MODULE}, ?MODULE, [Nodes, Options], []).
...
%% <li>`rtmp :: string()'</li>
%% </ul>
-spec(start_link/2 :: (Nodes :: list(atom()), Options :: start_opts()) -> {'ok', pid()}).
start_link(Nodes, [Head | _Tail] = Options) when is_tuple(Head) ->
	gen_server:start_link({local, ?MODULE}, ?MODULE, [Nodes, Options], []).
...
hold_outbound(Pid) ->
	gen_server:call(?MODULE, {hold_outbound, Pid}).

unhold_outbound(Pid) ->
	gen_server:call(?MODULE, {unhold_outbound, Pid}).

stop_outbound(Pid) ->
	gen_server:call(?MODULE, {stop_outbound, Pid}).
...
-spec(new_voicemail/7 :: (Node :: atom(), UUID :: string(), File :: string(), Queue :: string(), Priority :: pos_integer(), Client :: #client{} | string(), Info :: list()) -> 'ok').
new_voicemail(Node, UUID, File, Queue, Priority, Client, Info) ->
	gen_server:cast(?MODULE, {new_voicemail, Node, UUID, File, Queue, Priority, Client, Info}).

-spec(new_conference/7 :: (ConfId :: term(), Call :: #call{}, RingUuid :: string(), Queue :: string(), Priority :: pos_integer(), Client :: #client{} | string(), Info :: list()) -> 'ok').
new_conference(ConfId, Call, RingUuid, Queue, Priority, Client, Info) ->
	gen_server:cast(?MODULE, {new_conference, ConfId, Call, RingUuid, Queue, Priority, Client, Info}).
...
do_dial_string([${ | _] = DialString, Destination, Options) ->
    ...
    DialStr = string:strip(D1, left, ${ ),
    Res = lists:flatten(["{", string:join(EscapedOptions, ","), ",",DialStr]),
    Res;
...
do_dial_string(DialString, Destination, Options) ->
	...
	EscapedOptions = [re:replace(Option, "([^\\\\]),", "\\1\\\\,", [{return, list}, global]) ||
		Option <- Options],
	D1 = re:replace(DialString, "\\$1", Destination, [{return, list}]),
    Res = lists:flatten(["{", string:join(EscapedOptions, ","), "}", D1]),
    Res.

do_dial_string_without_sip_opts(DialString, Destination, Options) ->
	NoSipOptsDS = re:replace(DialString, <<"\\$1[\\S]+">>, <<"$1">>, [{return, list}]),
	do_dial_string(NoSipOptsDS, Destination, Options).
...
-spec recover_call(UUID :: string(), OldNode :: atom(), Queue :: string()) -> {ok, pid()} | {error, any()}.
recover_call(UUID, FSNode, Queue) ->
	gen_server:call(?MODULE, {recover_call, UUID, FSNode, Queue}).

transfer_to_node(Node, UUID, GenMediaArgs) ->
	lager:info("Tranferring"),
	gen_server:call({?MODULE, Node}, {transfer_to_node, UUID, GenMediaArgs}).
...
%% @private
init([Nodes, Options]) ->
	Nodename = get_nodename(Nodes),
	lager:info("Starting freeswitch_media_manager with nodename ~p", [Nodename]),
	process_flag(trap_exit, true),
	DialString = proplists:get_value(dialstring, Options, ""),
	CustomHeadersString = proplists:get_value(custom_headers, Options, ""),
	CustomHeaders = case CustomHeadersString of
		"" -> [];
		_  -> re:split(CustomHeadersString, ",", [{return, list}])
	end,
	monitor_node(Nodename, true),
	{Domain, Listenpid, DomainPid, FetchUserOpts} = case net_adm:ping(Nodename) of
		pong ->
			[net_adm:ping(OtherNode) || OtherNode <- Nodes, OtherNode =/= Nodename],
	...
	{ok, #state{nodename=Nodename, domain = Domain, dialstring = DialString,
		...
		fetch_domain_user = FetchUserOpts, custom_headers = CustomHeaders}}.
...
handle_call({initiate_outbound_call, {AgentLogin, Apid}, Client}, _From,
	...
	{reply, Reply, State};
handle_call({hold_outbound, Pid}, _From,
	#state{freeswitch_up = FS} = State) when FS == true ->
	Reply = freeswitch_outbound:hold(Pid),
	{reply, Reply, State};
handle_call({unhold_outbound, Pid}, _From,
	#state{freeswitch_up = FS} = State) when FS == true ->
	Reply = freeswitch_outbound:unhold(Pid),
	{reply, Reply, State};
handle_call({stop_outbound, Pid}, _From,
	#state{freeswitch_up = FS} = State) when FS == true ->
	Reply = freeswitch_outbound:stop(Pid),
	{reply, Reply, State};
...
handle_call(get_default_domain, _From, State) ->
	...
	{reply, Reply, State};
handle_call({transfer_to_node, UUID, GenMediaArgs}, _From, #state{call_dict = Dict} = State) ->
	lager:notice("call with pid ~s failing over on node ~p", [UUID, node()]),
	case dict:find(UUID, Dict) of
		{ok, Pid} ->
			Pid;
		error ->
			case gen_media:recover(GenMediaArgs) of
				{ok, Pid} ->
					link(Pid),
					{reply, {ok, Pid}, State#state{call_dict = dict:store(UUID, Pid, Dict)}};
				Else ->
					lager:warning("Error setting up new call ~s:  ~p", [UUID, Else]),
					{reply, Else, State}
			end
	end;
handle_call({recover_call, UUID, Node, Queue}, _From, #state{call_dict = Dict} = State) ->
	lager:notice("call ~s failing over on node ~p, queue ~p", [UUID, Node, Queue]),
	case dict:find(UUID, Dict) of
		{ok, Pid} ->
			Pid;
		error ->
			case freeswitch_media:start(Node, {State#state.dialstring, State#state.custom_headers}, UUID, Queue) of
				{ok, Pid} ->
					link(Pid),
					{reply, {ok, Pid}, State#state{call_dict = dict:store(UUID, Pid, Dict)}};
				Else ->
...
handle_cast({channel_destroy, UUID}, #state{call_dict = Dict} = State) ->
	...
	gproc:send({p, g, {channel_destroy_event, UUID}}, {channel_destroy_event, UUID}),
	{noreply, State};
...
handle_cast({new_voicemail, Node, UUID, File, Queue, Priority, Client, Info}, State) ->
	{ok, Pid} = freeswitch_voicemail:start(Node, UUID, File, Queue, Priority, Client, Info),
	link(Pid),
	{noreply, State};
handle_cast({new_conference, ConfId, Call, RingUuid, Queue, Priority, Client, Info}, #state{nodename = Node} = State) ->
	{ok, Pid} = freeswitch_conference:start(ConfId, Node, Call, RingUuid, Queue, Priority, Client, Info),
	link(Pid),
	{noreply, State};
...
%%--------------------------------------------------------------------
%% @private
handle_info({freeswitch_sendmsg, "inivr "++Data}, #state{call_dict = Dict} = State) ->
	[StrNode, UUID] = string:tokens(Data, " "),
	Node = list_to_atom(StrNode),
	lager:notice("call ~s entered IVR on node ~p", [UUID, Node]),
	case dict:find(UUID, Dict) of
		{ok, Pid} ->
			Pid;
		error ->
			case freeswitch_media:start(Node, {State#state.dialstring, State#state.custom_headers}, UUID) of
				{ok, Pid} ->
					link(Pid),
...
handle_info({get_pid, UUID, Ref, From}, #state{call_dict = Dict} = State) ->
	case dict:find(UUID, Dict) of
		{ok, Pid} ->
			...
			{noreply, State};
		error ->
			case freeswitch_media:start(State#state.nodename, {State#state.dialstring, State#state.custom_headers}, UUID) of
				{ok, Pid} ->
					From ! {Ref, Pid},
					link(Pid),
					{noreply, State#state{call_dict = dict:store(UUID, Pid, Dict)}};
...
%% @private
get_nodename(Nodes) ->
	%% Check if the local freeswitch is included in nodes
	%% else use the first node
	ReachNodeStr = atom_to_list(node()),
	[_, Domain] = string:tokens(ReachNodeStr, "@"),
	LocalCnode = list_to_atom("freeswitch@" ++ Domain),
	case lists:member(LocalCnode, Nodes) of
		true ->
			LocalCnode;
		false ->
			case Nodes of
...
listener(Node) ->
	receive
		{event, [UUID | Event]} ->
			...
			lager:debug("Received event ~p for ~p", [Ename, UUID]),
			case Ename of
				"CHANNEL_DESTROY" ->
