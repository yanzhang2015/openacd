...
%% License
-callback get_license_seats() -> {ok, number()} | none.

%% Permissions
-callback has_permission(PermProfile :: atom(), PermName :: binary()) -> {ok, true} | {ok, false} | {error, any()}.

-define(STORE,
	{ok, _Store} = application:get_env(reach_core, agent_auth_storage),
	_Store).
...
start() ->
	St = case application:get_env(reach_core, agent_auth_storage) of
		{ok, St0} ->
			St0;
		_ ->
			St0 = ?DEFAULT_STORAGE,
			application:set_env(openacd, agent_auth_storage, St0),
			St0
	end,
	St:start().
...
get_license_seats() ->
  ?STORE:get_license_seats().

%% @doc Check if a permission is enabled in a certain permission profile
-spec has_permission(PermProfile :: atom(), PermName :: binary()) -> {ok, true} | {ok, false} | {error, any()}.
has_permission(PermProfile, PermKey) ->
  ?STORE:has_permission(PermProfile, PermKey).
