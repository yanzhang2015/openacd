...
start(AgentRec, CallRec, EndpointData, InitState, EventManager, ChanId) ->
	gen_fsm:start(?MODULE, [AgentRec, CallRec, EndpointData, InitState, EventManager, ChanId], []).
...
start_link(AgentRec, CallRec, EndpointData, InitState, EventManager, ChanId) ->
	gen_fsm:start_link(?MODULE, [AgentRec, CallRec, EndpointData, InitState, EventManager, ChanId], []).
...
get_media(Apid) ->
	gen_fsm:sync_send_all_state_event(Apid, get_media).
...
hold_update(Apid, HoldStatus) ->
	gen_fsm:send_event(Apid, {hold_update, HoldStatus}).
...
%% @doc Request a conference to a queue
-spec(conference_to_queue/3 :: (Pid :: pid(), QueueName :: string(), Opts :: list()) -> ok | error).
conference_to_queue(Pid, QueueName, Opts) ->
	gen_fsm:sync_send_event(Pid, {conference_to_queue, QueueName, Opts}).

%% @doc Request a conference to an agent
-spec(conference_to_agent/2 :: (Pid :: pid(), AgentLogin :: string()) -> ok | error).
conference_to_agent(Pid, AgentLogin) ->
	gen_fsm:sync_send_event(Pid, {conference_to_agent, AgentLogin}).

conference_to_outband(Pid, Destination) ->
	gen_fsm:sync_send_event(Pid, {conference_to_outband, Destination}).

%% @doc Remove leg from conference
-spec(remove_from_conference/2 :: (Pid :: pid(), LegId :: string()) -> ok | error).
remove_from_conference(Pid, LegId) ->
	gen_fsm:sync_send_event(Pid, {remove_from_conference, LegId}).
...
init([Agent, Call, Endpoint, StateName, EventManager, ChanId]) ->
	...
	gproc:reg({n, g, {?MODULE, Agent#agent.login, ChanId}}),
	gen_event:notify(EventManager, {channel_feed, {initiated_channel, os:timestamp(), self(), Call}}),
	MediaMon = erlang:monitor(process, Call#call.source),
	case StateName of
		prering when is_record(Call, call); Call =:= undefined ->
			case start_endpoint(Endpoint, Agent, Call) of
				{ok, Pid} ->
					lager:debug("Starting in prering", []),
					conn_cast(Agent, set_channel_msg(State#state.channel_id, prering, Call)),
					cpx_agent_event:agent_channel_init(Agent, self(), prering, Call, Now),
					update_agent_state(Agent, prering),
					{ok, prering, State#state{endpoint = Pid, state_data = update_call(prering, Call), media_mon = MediaMon}};
				{error, Error} ->
					{stop, {error, Error}}
			end;
		...
		precall when is_record(Call, call) ->
			...
			conn_cast(Agent, set_channel_msg(State#state.channel_id, precall, Call)),
			cpx_agent_event:agent_channel_init(Agent, self(), precall, Call, Now),
			update_agent_state(Agent, precall),
			{ok, precall, State#state{state_data = update_call(precall, Call), media_mon = MediaMon}};
		ringing when is_record(Call, call) ->
			% TODO tell media to ring
			lager:debug("Starting in ringing", []),
			conn_cast(Agent, set_channel_msg(State#state.channel_id, ringing, Call)),
			cpx_agent_event:agent_channel_init(Agent,self(),ringing, Call, Now),
			update_agent_state(Agent, ringing),
			{ok, ringing, State#state{state_data = update_call(ringing, Call), media_mon = MediaMon}};
		_ ->
...
prering({ringing, Call}, From, #state{agent_rec = Agent} = State) ->
	...
	conn_cast(State#state.agent_connection, set_channel_msg(State#state.channel_id, ringing, Call)),
	cpx_agent_event:change_agent_channel(Agent, self(), ringing, Call, Now),
	update_agent_state(Agent, ringing),
	{reply, ok, ringing, update_state(State, Call, prering, ringing)};

prering(stop, _From, #state{endpoint = Ep} = State) ->
  cpx_endpoint:hangup(Ep),
  lager:info("Received stop while prering"),
...
ringing(oncall, {Conn, _}, #state{agent_rec = Agent, agent_connection = Conn, endpoint = Ep, state_data = #call{ring_path = inband}} = State) ->
	...
	case gen_media:oncall(Media) of
		ok ->
			conn_cast(Conn, set_channel_msg(State#state.channel_id, oncall, Call)),
			cpx_agent_event:change_agent_channel(Agent, self(), oncall, Call, Now),
			update_agent_state(Agent, oncall),
			...
			lager:debug("Moving from ringing to oncall state", []),
			{reply, ok, oncall, update_state(NewState, Call, ringing, oncall)};
		Else ->
			lager:warning("Didn't go oncall:  ~p", [Else]),
			{reply, {error, Else}, ringing, State}
	end;
...
ringing({oncall, #call{id = Id}}, _From, #state{agent_rec = Agent, state_data = #call{id = Id} = Call} = State) ->
	Now = ouc_time:now(),
	lager:debug("Moving from ringing to oncall state", []),
	conn_cast(State#state.agent_connection, set_channel_msg(State#state.channel_id, oncall, Call)),
	cpx_agent_event:change_agent_channel(Agent, self(), oncall, Call, Now),
	update_agent_state(Agent, oncall),
	{reply, ok, oncall, update_state(State, Call, ringing, oncall)};

ringing(stop, _From, #state{endpoint = Ep} = State) ->
	cpx_endpoint:hangup(Ep),
	{stop, normal, ok, State};
...
precall({oncall, #call{client = Client} = Call}, _From, #state{agent_rec = Agent, state_data = Client} = State) ->
	...
	cpx_agent_event:change_agent_channel(Agent, self(), oncall, Call, Now),
	update_agent_state(Agent, oncall),
	{reply, ok, oncall, update_state(State, Call, precall, oncall)};
...
precall({mediapush, From, Callrec, Data}, #state{state_data = #call{source = From}, agent_connection = Conn} = State) when is_pid(Conn) ->
	Self = self(),
	conn_cast(Conn, {mediapush, Self, State#state.channel_id, Callrec, Data}),
...
oncall(warmtransfer_hold, _From, #state{state_data = Call} = State) ->
	...
	conn_cast(State#state.agent_connection, set_channel_msg(State#state.channel_id, warmtransfer_hold, Call)),
	{reply, ok, warmtransfer_hold, update_state(State, Call, oncall, warmtransfer_hold)};
...
oncall({wrapup, #call{id = Id}=Call}, {From, _Tag}, #state{agent_rec = Agent, state_data = #call{id = Id}} = State) ->
	Now = ouc_time:now(),
	lager:info("Start wrapup for ~p", [Id]),
	...
	case Call#call.wrapup_enabled of
		true ->
			case Call#call.source of
				From ->
					...
					conn_cast(State#state.agent_connection, set_channel_msg(State#state.channel_id, wrapup, Call)),
					cpx_agent_event:change_agent_channel(Agent, self(), wrapup, Call, Now),
					update_agent_state(Agent, wrapup),
					prep_autowrapup(Call),
					{reply, ok, wrapup, update_state(State, Call, oncall, wrapup)};
				_CallSource ->
		...
		false ->
			...
			try gen_media:wrapup(CallPid) of
				...
				exit:Err ->
					lager:info("gen_media: ~p cannot wrapup due to ~p, proceeding anyway", [CallPid, Err]),
					{stop, normal, ok, State}
			end
	end;
...
oncall(unhold, _From, #state{state_data = Call} = State) ->
	...
	oncall({hold_update, undefined}, State),
	{reply, ok, oncall, State};
...
oncall({conference_to_queue, Queue, Opts}, _From, #state{state_data = Call} = State) ->
	MediaPid = Call#call.source,
	Reply = gen_media:conference_to_queue(MediaPid, Queue, Opts),
	{reply, Reply, oncall, State#state{in_conference = true}};

oncall({conference_to_agent, AgentLogin}, _From, #state{state_data = Call} = State) ->
	MediaPid = Call#call.source,
	Reply = gen_media:conference_to_agent(MediaPid, AgentLogin),
	{reply, Reply, oncall, State#state{in_conference = true}};

oncall({conference_to_outband, Destination}, _From,
	#state{state_data = Call, agent_rec = AgentRec} = State) ->
	MediaPid = Call#call.source,
	Reply = gen_media:conference_to_outband(MediaPid, Destination, AgentRec),
	{reply, Reply, oncall, State#state{in_conference = true}};

oncall({remove_from_conference, LegId}, _From, #state{state_data = Call} = State) ->
	MediaPid = Call#call.source,
	Reply = gen_media:remove_from_conference(MediaPid, LegId),
...
oncall({hold_update, HoldStatus}, State) ->
	Hold = case HoldStatus of
		hold -> onhold;
		undefined -> offhold;
		conference_onhold -> conference_onhold;
		conference_offhold -> conference_offhold;
		_ -> undefined
	end,
	ChanId = State#state.channel_id,
	conn_cast(State#state.agent_connection, {hold_update, ChanId, Hold}),
	{next_state, oncall, State};
...
warmtransfer_hold(oncall, _From, #state{state_data = Call} = State) ->
	...
	conn_cast(State#state.agent_connection, set_channel_msg(State#state.channel_id, oncall, Call)),
	{reply, ok, oncall, update_state(State, Call, warmtransfer_hold, oncall)};
warmtransfer_hold({warmtransfer_3rd_party, Data}, _From, #state{state_data = Call} = State) ->
	...
	conn_cast(State#state.agent_connection, set_channel_msg(State#state.channel_id, warmtransfer_3rd_party, Call)),
	{reply, ok, warmtransfer_3rd_party, {update_state(State, Call, warmtransfer_hold, warmtransfer_3rd_party), Data}};
warmtransfer_hold(wrapup, _From, #state{state_data = Call} = State) ->
	lager:debug("Moving from warmtransfer_hold to wrapup", []),
	conn_cast(State#state.agent_connection, set_channel_msg(State#state.channel_id, wrapup, Call)),
	{reply, ok, wrapup, update_state(State, Call, warmtransfer_hold, wrapup)};
...
warmtransfer_3rd_party(warmtransfer_hold, _From, #state{state_data = {Call, _}} = State) ->
	...
	conn_cast(State#state.agent_connection, set_channel_msg(State#state.channel_id, warmtransfer_hold, Call)),
	{reply, ok, warmtransfer_hold, update_state(State, Call, warmtransfer_3rd_party, warmtransfer_hold)};
warmtransfer_3rd_party(oncall, _From, #state{state_data = {Call, _}} = State) ->
	...
	conn_cast(State#state.agent_connection, set_channel_msg(State#state.channel_id, oncall, Call)),
	{reply, ok, oncall, update_state(State, Call, warmtransfer_3rd_party, oncall)};
warmtransfer_3rd_party(wrapup, _From, #state{state_data = {Call, _}} = State) ->
	...
	conn_cast(State#state.agent_connection, set_channel_msg(State#state.channel_id, wrapup, Call)),
	{reply, ok, wrapup, update_state(State, Call, warmtransfer_3rd_party, wrapup)};
...
handle_sync_event(get_media, _From, StateName, State) ->
	#call{source = Media} = _Call = State#state.state_data,
	{reply, {ok, Media}, StateName, State};
...
handle_sync_event({set_connection, Pid}, _From, StateName, #state{agent_connection = _AgentConn, state_data = Call} = State) ->
	conn_cast(Pid, set_channel_msg(State#state.channel_id, StateName, Call)),
...
handle_info({'DOWN', MediaMon, process, Media, _}, oncall, #state{media_mon = MediaMon,
		state_data = #call{source = Media} = Call, agent_rec = Agent} = State) ->
	lager:info("Call process ~p down, agent channel ~p going into wrapup", [Media, self()]),
	case Call#call.wrapup_enabled of
		true ->
			Now = ouc_time:now(),
			conn_cast(State#state.agent_connection, set_channel_msg(State#state.channel_id, wrapup, Call)),
			...
			update_agent_state(Agent, wrapup),
			prep_autowrapup(Call),
			{next_state, wrapup, update_state(State, Call, oncall, wrapup)};
		_ ->
			{stop, normal, State}
	end;
...
terminate(_Reason, StateName, State) ->
	lager:debug("Agent channel terminate: ~p", [self()]),
	case State#state.endpoint of
			undefined ->
				lager:notice("Endpoint already stopped, skipping hangup", []);
			_ ->
				cpx_endpoint:hangup(State#state.endpoint),
				receive
					{'DOWN', _EndpointRef, process, EndpointPid, Reason} ->
						lager:info("Endpoint ~p terminated with reason ~p", [EndpointPid, Reason]),
						ok
				after 2000 ->
						lager:warning("Timed out waiting for endpoint ~p down...", [State#state.endpoint]),
						ok
				end
	end,
	...
	NewState = update_state(State, State#state.state_data, StateName, stop),
	Call = NewState#state.state_data,
	update_agent_state(Agent, undefined),
...
init_gproc_prop(State, PrevChannelState, ChannelState) ->
	...
	gproc:reg({p, g, cpx_agent_channel}, Prop),
...
set_gproc_prop(State, PreviousStateName, StateName) ->
	...
	gproc:set_value({p, g, cpx_agent_channel}, Prop),
...
get_agent_channel_prop(FsmState, ChannelState) ->
	...
	CallId = get_call_prop(call_id, FsmState#state.state_data),
	StateChanges = get_call_prop(state_changes, FsmState#state.state_data),
	ChanId = FsmState#state.channel_id,
	#cpx_agent_channel_prop{login=Login, profile=Profile, type=Type,
		client=Client, callid=CallId, callerid=CallerId, channelid=ChanId,
		state=ChannelState, state_changes=StateChanges}.

get_call_prop(call_id, #call{id = Id}) ->
	Id;
get_call_prop(state_changes, #call{state_changes = StateChanges}) ->
	StateChanges;
get_call_prop(_, _) ->
	undefined.

update_state(State, Call, PrevSt, NewSt) ->
	NewCall = update_call(NewSt, Call),
	NewState = State#state{state_data = NewCall},
	set_gproc_prop(NewState, PrevSt, NewSt),
	NewState.

-spec update_call(NewSt :: atom(), Call :: #call{} | {#call{}, term()}) -> Call :: #call{} | {#call{}, term()}.
update_call(NewSt, #call{state_changes = Changes} = Call) ->
	UpdatedChanges = [{NewSt, os:timestamp()} | Changes],
	Call#call{state_changes = UpdatedChanges};
update_call(_, CallData) ->
	CallData.

-spec set_channel_msg(ChanId::integer(), NewSt::atom(), Call::#call{}) ->
	{set_channel, pid(), integer(), atom(), #call{}}.
set_channel_msg(ChanId, NewSt, Call) ->
	{set_channel, self(), ChanId, NewSt, update_call(NewSt, Call)}.

try_wrapup(State, Now) ->
	Call = State#state.state_data,
	CallPid = Call#call.source,
	Agent = State#state.agent_rec,
	InConf = State#state.in_conference,

	{Rep, Next} = case InConf of
		false ->
			try gen_media:wrapup(CallPid) of
				ok ->
					lager:debug("Moving from oncall to wrapup", []),
	...
	State1 = case Next of
		wrapup ->
			...
			update_agent_state(Agent, wrapup),
			prep_autowrapup(Call),
			update_state(State, Call, oncall, wrapup);
		_ ->
			State
	end,
...
force_wrapup(State, Now) ->
	...
	{ok, wrapup, update_state(State, Call, oncall, wrapup)}.

handle_endpoint_exit(ringing, State, {shutdown, call_expired}) ->
	...
	{stop, {shutdown, ring_init_failed}, State};
...
handle_endpoint_exit(StName, State, Reason) ->
	lager:info("Exit of endpoint due to ~p while ~p. exit", [Reason, StName]),
	{stop, Reason, State}.
...
handle_transfer_wrapup(TransferFun, From, #state{agent_rec=Agent,
	...
	case TransferFun() of
		ok ->
			...
			% conn_cast(AgentConn, set_channel_msg(State#state.channel_id, wrapup, Call)),
			% cpx_agent_event:change_agent_channel(Agent, self(), wrapup, Call, Now),
			% prep_autowrapup(Call),
...
update_agent_state(Agent, State) ->
	...
	agent:set_channel(AgentPid, self(), State).
