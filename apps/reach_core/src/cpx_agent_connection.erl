...
login(Username, Password, Opts) ->
	case agent_auth:auth(Username, Password) of
		{ok, Auth} ->
			start_agent_with_auth(Auth, Opts);
		_ ->
			{error, deny}
	end.
...
init(Agent) ->
	gproc:reg({p, g, ?MODULE}, Agent#agent.login),
...
handle_cast({mediapush, ChanPid, ChanId, Call, Data}, State) ->
	...
	case Data of
		...
		{channel_playback_update, Props} ->
			Json = {struct,  [
				...
				{<<"data">>, {struct, [
					{<<"channelid">>, encode_ch_id(ChanId)} | Props]}}]},
			{ok, Json, State};
...
handle_cast({set_channel, _Pid, ChanId, ChanState, Call}, State) ->
	Headjson = {struct, [
		{<<"command">>, <<"setchannel">>},
		{<<"state">>, ChanState},
		{<<"statedata">>, encode_call(Call)},
		{<<"channelid">>, encode_ch_id(ChanId)}
	]},
	{ok, Headjson, State};

handle_cast({hold_update, ChanId, HoldStatus}, State) ->
	lager:debug("received hold_update on channel ~p with hold Status ~p", [ChanId, HoldStatus]),
	Props0 = [{<<"event">>, hold_update}, {<<"channelid">>, list_to_binary(ChanId)}, {<<"status">>, HoldStatus}],
	{ok, {struct, Props0}, State};

handle_cast({channel_died, ChanId, NewAvail, Time}, State) ->
	Resp = case ChanId of
		none ->
			undefined;
		_ ->
...
encode_call(Call) ->
	...
	{struct, [
		...
		{<<"initial_skills">>, cpx_json_util:enc_skills(Call#call.initial_skills)},
		...
		{<<"type">>, Call#call.type},
		{<<"media_type">>, Call#call.media_type},
...
encode_ch_id(Id) when is_integer(Id) ->
	IdBin = list_to_binary(integer_to_list(Id)),
	<<"ch", IdBin/binary>>;
encode_ch_id(Id) when is_list(Id) ->
	list_to_binary(Id).
