...
-export([
	...
	play/2,
	pause/1,
	conference_to_queue/2,
	conference_to_queue/3,
	conference_to_agent/2,
	conference_to_outband/3,
	remove_from_conference/2,
	leave_conference/1,
	transfer_to_node/3,
	run_ivr/2
]).
...
ring(Genmedia, {_Agent, Apid} = A, Qcall, Timeout) when is_pid(Apid) ->
	lager:info("Ring invoked for gen_media ~p to agent ~p by ~p", [Genmedia, _Agent, self()]),
...
voicemail(Genmedia) ->
	voicemail(Genmedia, undefined).
voicemail(Genmedia, CookArgs) ->
	gen_fsm:sync_send_event(Genmedia, ?GM(voicemail, CookArgs)).

%% @doc Send the passed `pid() Genmedia' to voicemail if inqueue, on ring failure if inqueue_ringing
-spec(try_voicemail/1 :: (Genmedia :: pid()) -> 'ok' | 'invalid').
try_voicemail(Genmedia) ->
	try_voicemail(Genmedia, undefined).
try_voicemail(Genmedia, CookArgs) ->
	gen_fsm:sync_send_event(Genmedia, ?GM(try_voicemail, CookArgs)).
...
announce(Genmedia, Annouce) ->
	announce(Genmedia, Annouce, undefined).
announce(Genmedia, Annouce, CookArgs) ->
	gen_fsm:sync_send_event(Genmedia, ?GM(announce, {Annouce, CookArgs})).
...
set_queue_key(Genmedia, Key) ->
	gen_fsm:send_all_state_event(Genmedia, ?GM(set_queue_key, Key)).


%%-------------------------------------------------------------------------------------
-spec(set_queue_details(Genmedia :: pid(), Key :: tuple(), Skills :: list()) -> 'ok').
%% @doc Sets details about the reach_vqueue to gen_media as the queue key and
%%   the skills that were eventually set for the call. Sends an asynchronous
%%   message to gen_media that will be handled by the appropriate handle_event
%%   function.
%% @end
%%-------------------------------------------------------------------------------------
set_queue_details(Genmedia, Key, Skills) ->
  gen_fsm:send_all_state_event(Genmedia, ?GM(set_queue_details, {Key, Skills})).
...
get_monitor_leg(Genmedia) ->
	gen_fsm:sync_send_all_state_event(Genmedia, ?GM(get_monitor_leg)).

-spec(set_skills/2 :: (Genmedia :: pid(), Skills :: [atom() | {atom(), any()}]) -> 'ok').
set_skills(Genmedia, Skills) ->
	gen_fsm:send_all_state_event(Genmedia, ?GM(set_skills, Skills)).

-spec(add_skills/2 :: (Genmedia :: pid(), Skills :: [atom() | {atom(), any()}]) -> 'ok').
add_skills(Genmedia, Skills) ->
	add_skills(Genmedia, Skills, undefined).
add_skills(Genmedia, Skills, CookArgs) ->
	gen_fsm:send_all_state_event(Genmedia, ?GM(add_skills, {Skills, CookArgs})).

-spec(remove_skills/2 :: (Genmedia :: pid(), Skills :: [atom() | {atom(), any()}]) -> 'ok').
remove_skills(Genmedia, Skills) ->
	remove_skills(Genmedia, Skills, undefined).
remove_skills(Genmedia, Skills, CookArgs) ->
	gen_fsm:send_all_state_event(Genmedia, ?GM(remove_skills, {Skills, CookArgs})).

transfer_outband(Genmedia, Addr) ->
	transfer_outband(Genmedia, Addr, undefined).
transfer_outband(Genmedia, Addr, CookArgs) ->
	gen_fsm:sync_send_event(Genmedia, ?GM(transfer_outband, {Addr, CookArgs})).
...
hold_update(Genmedia, HoldState) ->
	gen_fsm:sync_send_event(Genmedia, ?GM({hold_update, HoldState})).
...
%% @doc Queues a conference request
-spec(conference_to_queue/3 :: (Genmedia :: pid(), QueueName :: string(), Opts :: list()) -> 'ok' | 'error').
conference_to_queue(Genmedia, QueueName, Opts) ->
	gen_fsm:sync_send_event(Genmedia, ?GM({conference_to_queue, QueueName, Opts})).

%% @doc Queues a conference request with the default opts
-spec(conference_to_queue/2 :: (Genmedia :: pid(), QueueName :: string()) -> 'ok' | 'error').
conference_to_queue(Genmedia, QueueName) ->
	conference_to_queue(Genmedia, QueueName, []).

%% @doc Queues a conference request to a specific agent
-spec(conference_to_agent/2 :: (Genmedia :: pid(), AgentLogin :: string()) -> 'ok' | 'error').
conference_to_agent(Genmedia, AgentLogin) ->
	Opts = [{skills, [{'_agent', AgentLogin}]}],
	conference_to_queue(Genmedia, ?TRANSFER_QUEUE, Opts).

conference_to_outband(Genmedia, Destination, AgentRec) ->
	gen_fsm:sync_send_event(Genmedia, ?GM({conference_to_outband, Destination, AgentRec, []})).

%% @doc Removes a conference leg
-spec(remove_from_conference/2 :: (Genmedia :: pid(), LegId :: any()) -> 'ok' | 'error').
remove_from_conference(Genmedia, LegId) ->
	gen_fsm:sync_send_event(Genmedia, ?GM({remove_from_conference, LegId})).

%% @doc Registers another media as a conference media
-spec(leave_conference/1 :: (Genmedia :: pid()) -> 'ok' | 'error').
leave_conference(Genmedia) ->
	gen_fsm:sync_send_event(Genmedia, ?GM(leave_conference)).

transfer_to_node(Genmedia, Node, VqueueKey) ->
	gen_fsm:sync_send_all_state_event(Genmedia, ?GM({transfer_to_node, Node, VqueueKey})).

-spec(run_ivr/2 :: (Genmedia :: pid(), Args :: list()) -> ok).
run_ivr(Genmedia, Args) ->
	gen_fsm:send_all_state_event(Genmedia, ?GM(run_ivr, Args)).
...
recover(Args) ->
	gen_fsm:start(?MODULE, [{recover, Args}], []).
...
%% @private
init([{recover, [OldPid, VqueueKey, StateName, {BaseState, Internal}]}]) ->
	Info = [{reach_vqueue_key, VqueueKey}, {recovered_pid, OldPid}],
	lager:info("Recovering ~p with vqueue key ~p, state ~p", [OldPid, VqueueKey, StateName]),
	case gproc:whereis_name(VqueueKey) of
		Vqueue when is_pid(Vqueue) ->
			lager:info("Sending ~p to ~p", [{gen_media_recovery, OldPid, self()}, Vqueue]),
			Vqueue ! {gen_media_recovery, OldPid, self()};
		_ ->
			lager:info("Subscribing to gproc ~p", [{reach_vqueue_registered, VqueueKey}]),
			gproc:add_global_property({reach_vqueue_registered, VqueueKey}, subscribe)
	end,

	Call = BaseState#base_state.callrec,
	NewCall = Call#call{source = self()},
	Callback = BaseState#base_state.callback,
	Substate = BaseState#base_state.substate,
	NewSub = case erlang:function_exported(Callback, handle_recover, 3) of
		true ->
			{ok, S} = Callback:handle_recover(Call, Internal, Substate),
			S;
		_ ->
			Substate
	end,

	NewBase = BaseState#base_state{recovery_info = Info, callrec = NewCall, substate = NewSub},
	init_gproc_prop(StateName, NewBase),
	gproc:reg({n, g, {?MODULE, Call#call.id}}),
	{ok, StateName, {NewBase, Internal}};

init([Callback, Args]) ->
	InitTime = os:timestamp(),
	StateChanges = [{init, InitTime}],

	lager:debug("Initiating gen_media with callback ~p and args ~p", [Callback, Args]),
	case Callback:init(Args) of
		{ok, SubState, Ps} ->
			QueueN = ?get(queue, Ps),

			Call = ps_to_call(Ps, undefined, Callback, StateChanges),
			InConference = proplists:get_value(conference, Call#call.info, false),
			...
			lager:info("loginit called, queue name ~p", [QueueN]),
			cdr:cdrinit(Call),
			cdr:loginit(Call),
			...
			{NSt,NBaseSt, NIntSt} = case QueueN of
				...
				_ ->
					lager:info("Queueing ~p to ~p", [Call, QueueN]),
...
inqueue(?GM(ring, {{Agent, Apid}, #queued_call{
		cook = Requester, skills = ESkills} = _QCall, _Timeout}), _From, {
		#base_state{callrec = Call} = BaseState,
		#inqueue_state{sent_to_voicemail = SentToVM} = Internal}) ->
	lager:debug("Queued call: ~p", [_QCall]),
	lager:warning("Ag+AgPid: (~p,~p)",[Agent, Apid]),
	...
	lager:info("Trying to ring ~p, ~p with ~p with timeout ~p", [Agent, Apid, Call1#call.id, Timeout]),
	case SentToVM of
		false ->
			try agent:prering(Apid, Call1) of
				{ok, RPid} ->
					...
					lager:debug("Trying to ring to agent ~p, state ~p", [Agent, BaseState2]),
		...
		_ ->
			{reply, ignored, inqueue, {BaseState1, Internal}}
	end;

inqueue(?GM(ring, {{_Agent, Apid}, QCall, _Timeout}), _From, State) ->
	lager:info("Ring to ~p deferred", [_Agent]),
	gen_server:cast(QCall#queued_call.cook, {ring_to, Apid, QCall}),
	{reply, deferred, inqueue, State};

inqueue(?GM(announce, {Announce, CookArgs}), _From, St) ->
	{#base_state{callrec = Call} = _BaseState, _Internal} = St,
	case CookArgs of
		undefined -> ok;
		_Else -> cdr:announce(Call, CookArgs)
	end,
	sync_call_cbk(inqueue, St, handle_announce, [Announce]);

inqueue(?GM(voicemail, CookArgs), _From, St) ->
	lager:info("Sending inqueue media ~p to voicemail", [self()]),
	Res = sync_call_cbk(inqueue, St, handle_voicemail, [], fun(#cbkr{cbk_res=ok}=R) ->
			priv_voicemail(St, CookArgs),
			R;
		(R) ->
			R
	end),
	case Res of
		{reply, ok, NextState, {Base, Internal}} ->
			{reply, ok, NextState, {Base, Internal#inqueue_state{sent_to_voicemail = true}}};
		Els ->
			Els
	end;

inqueue(?GM(try_voicemail, CookArgs), From, {BaseState, Internal}) ->
	case Internal#inqueue_state.sent_to_voicemail of
		false ->
			inqueue(?GM(voicemail, CookArgs), From, {BaseState, Internal});
		_ ->
			{reply, ok, inqueue, {BaseState, Internal}}
	end;

inqueue(?GM(transfer_outband, {Addr, CookArgs}), _From, { #base_state{
		callrec = Call} = _BaseState, #inqueue_state{queue_mon = Mon, queue_pid = {_QNom, QPid},
		cook = Cook, cook_mon = CookMon}} = St) ->
	erlang:demonitor(Mon),
	lager:info("Going to external URI, removing call ~p from queue ~p", [self(), QPid]),
	reach_vqueue:bgremove(QPid, self()),
	case Cook of
		_ when is_pid(Cook) ->
			erlang:demonitor(CookMon),
			reach_vqueue:remove_cook(QPid, Cook);
		_ ->
			ok
	end,
	lager:debug("gen_media:inqueue(transfer_outband): trying to transfer to ~p", [Addr]),
	cdr:outband_transfer(Call, {Addr, CookArgs}),
...
inqueue(?GM(set_queue, {QPid, Reset}), From, St) when is_pid(QPid) ->
	QName = reach_vqueue:get_name(QPid),
...
inqueue(?GM(leave_conference), From, {BaseState, Internal}) ->
    lager:info("Leaving conference while inqueue"),
    Callback = BaseState#base_state.callback,
    case erlang:function_exported(Callback, handle_leave_conference, 3) of
        true ->
            Substate = BaseState#base_state.substate,
            case Callback:handle_leave_conference(From, Internal, Substate) of
                {hangup, NewSub} ->
                    {stop, normal, ok,
                        {BaseState#base_state{substate = NewSub}, Internal}};
                {error, _, NewSub} ->
                    NewBase = BaseState#base_state{substate = NewSub},
                    {reply, invalid, {NewBase, Internal}}
            end;
        false ->
...
inqueue_ringing(?GM(announce, {Announce, CookArgs}), _From, {BaseState, Internal}) ->
	#base_state{callback = Callback, substate = InSubstate,
		callrec = Call} = BaseState,
...
inqueue_ringing(?GM(voicemail, _CookArgs), _From, {BaseState, Internal}) ->
	#base_state{callback = Callback, callrec = Call} = BaseState,
	lager:info("trying to send media ~p to voicemail", [Call#call.id]),
	send_to_voicemail_after_ring(Callback, Call, BaseState, Internal);

inqueue_ringing(?GM(try_voicemail), _From, {BaseState, Internal}) ->
	lager:info("Send to voicemail deferred until ring failure"),
	NewInternal = Internal#inqueue_ringing_state{
		voicemail_on_ring_failure = true},
	{reply, deferred, inqueue_ringing, {BaseState, NewInternal}};

inqueue_ringing(?GM(transfer_outband, {Addr, _}), _From, {BaseState, Internal}) ->
	#base_state{callback = Callback, callrec = Call} = BaseState,
	lager:debug("gen_media:inqueue_ringing(transfer_outband): trying to transfer ~p to ~p", [Call#call.id, Addr]),
...
inqueue_ringing(?GM(agent_oncall), {Apid, _Tag},
		...
		#inqueue_ringing_state{ring_pid = {Agent, Apid}, cook = Cook,
			cook_mon = CookMon, queue_pid = {_, QPid}} = Internal}) ->
	...
	case Callback:handle_answer(Apid, inqueue_ringing, Call, Internal, BaseState#base_state.substate) of
		{ok, NewState} ->
			...
			NewBase = case BaseState#base_state.init_agent of
				undefined ->
					BaseState#base_state{substate = NewState, state_changes = StateChanges, agent = AgentRec, init_agent = AgentRec#agent.login};
				_ ->
					BaseState#base_state{substate = NewState, state_changes = StateChanges, agent = AgentRec}
			end,
			...
			case Cook of
				_ when is_pid(Cook) ->
					catch erlang:demonitor(CookMon),
					reach_vqueue:remove_cook(QPid, Cook);
				_ ->
					ok
			end,
...
inqueue_ringing(?GM(agent_oncall), From, {BaseState, Internal}=St) ->
	Zt = inqueue_ringing,
	#base_state{callback = Callback, callrec = Call} = BaseState,
	#inqueue_ringing_state{ring_pid = {Agent, Apid}, ring_mon = Mon,
		cook = Cook, cook_mon = CookMon, queue_pid = {_, QPid}} = Internal,
	...
	case set_agent_state(Apid, [oncall, Call]) of
		...
		ok ->
			sync_call_cbk(Zt, St, handle_answer, [Apid], fun(#cbkr{cbk_res=ok}=R) ->
					...
					{_, _Qpid} = Internal#inqueue_ringing_state.queue_pid,
					% reach_vqueue:remove(Qpid, self()),
					StateChanges = [{oncall, os:timestamp()} | BaseState#base_state.state_changes],
					{ok, AgentRec} = agent_channel:get_agent(Apid),
					NewBase = case BaseState#base_state.init_agent of
						undefined ->
							(R#cbkr.base_state)#base_state{state_changes = StateChanges, init_agent = AgentRec#agent.login, agent = AgentRec};
						_ ->
							(R#cbkr.base_state)#base_state{state_changes = StateChanges, agent = AgentRec}
					end,
					set_gproc_prop(inqueue_ringing, oncall, NewBase),
					NewInternal = #oncall_state{
						oncall_pid = {Agent, Apid},
...
inqueue_ringing(?GM(ring, {{Agent, Apid}, _ChanType, takeover}),
	...
	gen_fsm:cancel_timer(Internal#inqueue_ringing_state.ringout),
	case catch agent_channel:set_state(Apid, ringing, BaseState#base_state.callrec) of
		{'EXIT', _} ->
			lager:info("Caught agent_channel exit when setting to ringing");
		_ ->
			ok
	end,
...
inqueue_ringing(?GM(leave_conference), From, {BaseState, Internal}) ->
	Callback = BaseState#base_state.callback,
	case erlang:function_exported(Callback, handle_leave_conference, 3) of
		true ->
			Substate = BaseState#base_state.substate,
...
inqueue_ringing(?GM(takeover_ring, {{Agent, Apid}, OutbandRinger}), {BaseState,
	...
	NewInternal = Internal#inqueue_ringing_state{ringout = undefined,
		outband_ring_pid = OutbandRinger},
	case erlang:function_exported(Callback, handle_ringing, 5) of
		true ->
			Substate = BaseState#base_state.substate,
			case Callback:handle_ringing(inqueue_ringing, Call, Agent,
					NewInternal, Substate) of
				{ok, NewSub} ->
					NewBase = BaseState#base_state{substate = NewSub},
					{next_state, inqueue_ringing, {NewBase, NewInternal}};
				{error, _Reason, NewSub} ->
					NewBase = BaseState#base_state{substate = NewSub},
					{next_state, inqueue_ringing, {NewBase, NewInternal}}
			end;
		false ->
...
oncall(?GM(queue, {Queue, Opts}), From, {BaseState, Internal}) ->
	#base_state{callback = Callback, callrec = Call, in_conference = InConf} = BaseState,
	...
	NewCallSegment = [MajorCallSegment + 1],
	FinalCall = case proplists:get_value(skills, Opts) of
		undefined ->
			Call#call{call_segment = NewCallSegment};
		NewSkills ->
			case proplists:get_value('_agent', NewSkills) of
				undefined ->
					QueueData = [{new_queue, Queue}, {new_call_segment, NewCallSegment}],
					cdr:queue_transfer(Call, QueueData);
				NewAgent ->
					AgentData = [{old_agent, Ocagent}, {new_agent, NewAgent}, {new_call_segment, NewCallSegment}],
					cdr:agent_transfer(Call, AgentData)
			end,
			Call#call{skills=NewSkills, call_segment = NewCallSegment}
	end,
	cdr:loginit(FinalCall),
	case InConf of
		true ->
			case oncall(?GM({conference_to_queue, Queue, [{transfer, true} | Opts]}), From,
					{BaseState, Internal}) of
				{reply, {ok, _}, oncall, {NewBase, NewInternal}} ->
					oncall(?GM(wrapup), From, {NewBase, NewInternal});
				_ ->
					{reply, error, oncall, {BaseState, Internal}}
			end;
		_ ->
			RequeuedCall = reprioritize_for_requeue(FinalCall),
			lager:info("Queueing ~p to ~p", [RequeuedCall, Queue]),
			case enqueue(Queue, RequeuedCall, BaseState#base_state{agent = undefined}) of
				{ok, {NewBase,
					  #inqueue_state{queue_pid = {_QN, Qpid}} = NewInternal}} ->
					async_set_agent_state(Apid, [wrapup, Call]),
					{ok, NewState} = Callback:handle_queue_transfer({Queue, Qpid}, oncall, FinalCall, NewInternal, NewBase#base_state.substate),
					cdr:wrapup(Call, Ocagent),
					cdr:hangup(Call, "agent"),
					erlang:demonitor(Mon),
					% cdr:queue_transfer(Call, Queue),
					% cdr:queue_transfer(NewCall, Queue),
					FinalBase = NewBase#base_state{
						substate = NewState},
					set_gproc_prop(oncall, inqueue, FinalBase),
					{reply, ok, inqueue, {FinalBase, NewInternal}};
				_ -> {reply, invalid, {BaseState, Internal}}
			end
	end;

oncall(?GM(transfer_outband, {Addr, _}), From, {BaseState, Internal} = St) ->
	#base_state{agent=Agent, callrec=Call, in_conference=InConf} = BaseState,
	#oncall_state{oncall_pid = {Ocagent, _Apid}} = Internal,
	case InConf of
		true ->
			case oncall(?GM({conference_to_outband, Addr, Agent, [{transfer, true}]}), From,
					{BaseState, Internal}) of
				{reply, {ok, _}, oncall, {NewBase, NewInternal}} ->
					oncall(?GM(wrapup), From, {NewBase, NewInternal});
				_ ->
		...
		_ ->
			cdr:outband_transfer(Call, Addr),
			cdr:wrapup(Call, Ocagent),
			cdr:hangup(Call, "agent"),
			lager:debug("gen_media:oncall(transfer_ouband): trying to transfer ~p to ~p", [Call#call.id, Addr]),
			sync_call_cbk(oncall, St, handle_transfer_outband, [Addr],
				fun(#cbkr{cbk_res=ok}=R) ->
					R#cbkr{action={stop, normal}};
				(R) -> R end)
	end;
...
oncall(?GM(wrapup), {Ocpid, _Tag} = From,
		{#base_state{init_callrec = Call} = BaseState,
	...
	InitialAgent = BaseState#base_state.init_agent,
	lager:debug("The agent corresponding to the first call leg for call ~p is ~p and call changes timestamps ~p", [Call#call.id, InitialAgent, StateChanges]),
	case erlang:function_exported(Callback, is_recording_enabled, 0) of
		true ->
			case Callback:is_recording_enabled() of
				true ->
					cpx_recordings:store_file(Call, StateChanges, InitialAgent);
				_ ->
					ok
			end;
		_ ->
...
oncall(?GM(queue, Queue), {Ocpid, _},
	...
	case priv_queue(Queue, reprioritize_for_requeue(Call), get_init(BaseState#base_state.state_changes), BaseState#base_state.queue_failover) of
		invalid ->
			{reply, invalid, State};
		{default, Qpid} ->
		...
		{Queue, Qpid} when is_pid(Qpid) ->
			...
			StateChanges = [{inqueue, os:timestamp()} | BaseState#base_state.state_changes],
			lager:debug("State changes in oncall(queue): ~p", [StateChanges]),
			NewBase = BaseState#base_state{substate = NewState, state_changes = StateChanges},
...
oncall(?GM({hold_update, HoldStatus}), _From, {BaseState,
	#oncall_state{oncall_pid = {_Ocagent, Apid}} =Internal}) ->
	agent_channel:hold_update(Apid, HoldStatus),
	Callback = BaseState#base_state.callback,
	{Reply, NewState} = Callback:handle_hold_update(HoldStatus, Internal, BaseState#base_state.substate),
	{reply, Reply, oncall, {BaseState#base_state{substate = NewState}, Internal}};
...
oncall(?GM({conference_to_queue, QueueName, Opts}), _From,
	{#base_state{next_call_segment = NextCallSegment} = BaseState,
	#oncall_state{oncall_pid = {Ocagent, _Apid}} = Internal}) ->

	lager:debug("Handling conference to queue with opts ~p ", [Opts]),
	Callback = BaseState#base_state.callback,
	Substate = BaseState#base_state.substate,
	Call = BaseState#base_state.callrec,
	CallSegment = Call#call.call_segment,
	LegId = integer_to_list(util:erl_now_int()),
	%% TODO just pass skills as arg

	{Reply, NewState, InConf} = case erlang:function_exported(Callback, handle_conference_to_queue, 6) of
		true ->
			NewCallSegment = [NextCallSegment|CallSegment],
			NewCall = case proplists:get_value(skills, Opts) of
				undefined ->
					Call#call{call_segment = NewCallSegment};
				NewSkills ->
					case proplists:get_value('_agent', NewSkills) of
						undefined ->
							QueueData = [{new_queue, QueueName}, {new_call_segment, NewCallSegment}],
							cdr:queue_conference(Call, QueueData);
						NewAgent ->
							AgentData = [{old_agent, Ocagent}, {new_agent, NewAgent}, {new_call_segment, NewCallSegment}],
							cdr:agent_conference(Call, AgentData)
					end,
					Call#call{skills = NewSkills, call_segment = NewCallSegment}
			end,
			Request = case proplists:get_value('_agent',
					proplists:get_value(skills, Opts, [])) of
				undefined ->
					{queue, QueueName};
				ConfAgent ->
					{agent, ConfAgent}
			...
			case Callback:handle_conference_to_queue({LegId, Request}, NewCall,
					QueueName, Opts, Internal, Substate) of
				{ok, NewSub} ->
					{{ok, LegId}, NewSub, true};
				{error, Err, NewSub} ->
					{{error, Err}, NewSub, false};
				Error ->
					lager:error("An error occured while transfering to a conference: ~p", [Error]),
					{{error, unexpected_result}, Substate, false}
			end;
		false ->
			{{error, not_supported}, Substate, false}
	end,
	erlang:demonitor(Internal#oncall_state.oncall_mon),
	{reply, Reply, oncall, {BaseState#base_state{
		next_call_segment = NextCallSegment + 1,
		in_conference = InConf, substate = NewState}, Internal}};

oncall(?GM({conference_to_outband, Destination, AgentRec, Opts}), _From, {BaseState, Internal}) ->
	Callback = BaseState#base_state.callback,
	Substate = BaseState#base_state.substate,
	Call = BaseState#base_state.callrec,
	ConfId = integer_to_list(util:erl_now_int()),
	% CallSegment = Call#call.call_segment,
	{Reply, NewState, InConf} = case erlang:function_exported(Callback, handle_conference_to_outband, 7) of
		true ->
			case Callback:handle_conference_to_outband(Call, Destination,
						AgentRec, ConfId, Opts, Internal, Substate) of
				{{ok, _Pid}, NewSub} ->
					{{ok, ConfId}, NewSub, true};
				{error, NewSub} ->
					{error, NewSub, false}
			end;
		false ->
	...
	erlang:demonitor(Internal#oncall_state.oncall_mon),
	{reply, Reply, oncall, {BaseState#base_state{in_conference = InConf, substate = NewState}, Internal}};

oncall(?GM({remove_from_conference, LegId}), _From, {#base_state{callrec=Call} = BaseState,
		Internal}) ->
	OrigId = Call#call.original_id,
	conference_manager:remove_leg(OrigId, LegId),
	{reply, ok, oncall, {BaseState, Internal}};

oncall(?GM(leave_conference), From,
		{#base_state{callrec = Call} = BaseState,
		#oncall_state{oncall_pid = {Ocagent, _}} = Oncall})
		when Call#call.media_path =:= inband ->
	Callback = BaseState#base_state.callback,
	cdr:wrapup(Call, Ocagent),
	case erlang:function_exported(Callback, handle_leave_conference, 3) of
		true ->
			case Callback:handle_leave_conference(From, Oncall, BaseState#base_state.substate) of
				{ok, NewState} ->
					StateChanges = [{wrapup, os:timestamp()} | BaseState#base_state.state_changes],
					NewBase = BaseState#base_state{substate = NewState, state_changes = StateChanges},
					set_gproc_prop(oncall, wrapup, NewBase),
...
oncall(?GM(agent_oncall), _From, {BaseState, Internal}) ->
  {reply, ok, oncall, {BaseState, Internal}};

oncall(Msg, From, State) ->
	lager:debug("Unhandled msg from oncall: ~p", [Msg]),
...
handle_sync_event(?GM(get_monitor_leg), _From, StateName, State) ->
	{BaseState, _Internal} = State,
	#base_state{
		substate = Substate,
		callback = Callback
	} = BaseState,
	Uuid = case erlang:function_exported(Callback, get_monitor_leg, 1) of
		true ->
			Callback:get_monitor_leg(Substate);
		false ->
			undefined
	end,
	{reply, {ok, Uuid}, StateName, State};
...
handle_sync_event(?GM({transfer_to_node, Node, VqueueKey}), _From, StateName, {BaseState, Internal}) ->
	lager:info("Transfering to node ~p, vqueue ID ~p in state ~p", [Node, VqueueKey, StateName]),
	Call = BaseState#base_state.callrec,
	Callback = BaseState#base_state.callback,
	gproc:unreg({n, g, {?MODULE, Call#call.id}}),
	ReinitArgs = [self(), VqueueKey, StateName, {BaseState, Internal}],
	case erlang:function_exported(Callback, handle_transfer_to_node, 3) of
		true ->
			Callback:handle_transfer_to_node(Node, ReinitArgs, BaseState#base_state.substate);
		false ->
			ok
	end,
	{stop, normal, ok, {BaseState, Internal}};
...
handle_event(?GM(set_skills, Skills), StateName, State) ->
	{BaseState, Internal} = State,
	Call = BaseState#base_state.callrec,
	NewCall = case Call#call.initial_skills of
	  [] -> Call#call{skills = Skills, initial_skills = Skills};
	  _ -> Call#call{skills = Skills}
	end,
	NewBase = BaseState#base_state{callrec = NewCall},
	set_gproc_prop(StateName, StateName, NewBase),
	{next_state, StateName, {NewBase, Internal}};

handle_event(?GM(add_skills, {Skills, CookArgs}), StateName, State) ->
	{BaseState, Internal} = State,
	Call = BaseState#base_state.callrec,
	...
	case CookArgs of
		undefined -> ok;
		_Else -> cdr:add_skills(Call, CookArgs)
	end,
	{next_state, StateName, {NewBase, Internal}};

handle_event(?GM(set_queue_key, Key), StateName, State) ->
	{BaseState, Internal} = State,
	NewBase = BaseState#base_state{queue_key = Key},
	{next_state, StateName, {NewBase, Internal}};

handle_event(?GM(set_queue_details, {Key,  Skills}), StateName, State) ->
	{BaseState,  Internal} = State,
	CallRecord = BaseState#base_state.callrec,
	NewBase = BaseState#base_state{queue_key = Key, callrec = CallRecord#call{skills = Skills}},
	{next_state, StateName, {NewBase, Internal}};

handle_event(?GM(remove_skills, {Skills, CookArgs}), StateName, State) ->
	{BaseState, Internal} = State,
	Call = BaseState#base_state.callrec,
	...
	NewBase = BaseState#base_state{callrec = NewCall},
	set_gproc_prop(StateName, StateName, NewBase),
	lager:info("in gen_media remove_skills, with cookArgs : ~p", [CookArgs]),
	case CookArgs of
		undefined -> ok;
		_Else -> cdr:remove_skills(Call, CookArgs)
	end,
	{next_state, StateName, {NewBase, Internal}};

handle_event(?GM(run_ivr, Args), StateName, State) ->
	{BaseState, Internal} = State,
	Call = BaseState#base_state.callrec,
	Callback = BaseState#base_state.callback,
	Substate = BaseState#base_state.substate,
	NewSub = case erlang:function_exported(Callback, handle_run_ivr, 4) of
		true ->
			{ok, S} = Callback:handle_run_ivr(Call, Args, Internal, Substate),
			S;
		_ ->
			Substate
	end,
...
handle_info({'DOWN', Ref, process, Pid, Info}, inqueue_ringing, {BaseState, Internal}) ->
	#inqueue_ringing_state{ring_pid = {Aname, Pid}, ring_mon = Ref,
		queue_pid = Queue, queue_mon = Qmon, cook_mon = Cmon, cook = Cook,
		voicemail_on_ring_failure = SendToVM} = Internal,
	#base_state{callrec = Call, callback = Callback, substate = Sub} = BaseState,
	...
	case SendToVM of
		true ->
			...
			lager:info("Sending inqueue_ringing media ~p to voicemail on ring failure", [self()]),
			{_, Reply, NextState, {NewBase, NewInternal}} = send_to_voicemail_after_ring(Callback, Call, BaseState#base_state{substate = NewSub}, Internal),
			case Reply of
				ok ->
					{next_state, NextState, {NewBase, NewInternal}};
				_ ->
					reach_vqueue:requeue(Q, BaseState#base_state.queue_key, self(), Call),
					RequeuedInternal = #inqueue_state{
						queue_mon = Qmon,
						queue_pid = Queue,
						cook = Cook,
						cook_mon = Cmon
					},
					{next_state, inqueue, {NewBase, RequeuedInternal}}
			end;
		_ ->
			reach_vqueue:requeue(Q, BaseState#base_state.queue_key, self(), Call),
			{ok, NewSub} = Callback:handle_ring_stop(inqueue_ringing, Call, Internal, Sub),
...
handle_info({'DOWN', Ref, process, Pid, _Info}, oncall_ringing, {BaseState,
	...
	{"default_queue", Qpid} = priv_queue("default_queue", reprioritize_for_requeue(Call), get_init(BaseState#base_state.state_changes), true),
	...
	NewBase = BaseState#base_state{substate = NewSub, callrec = Call#call{queue = "default_queue"}},
	#oncall_ringing_state{ring_pid = Rpid, ring_mon = Rmon} = Internal,
	NewInternal = #inqueue_ringing_state{
		ring_pid = Rpid, ring_mon = Rmon, queue_pid = {"default_queue", Qpid},
		queue_mon = Qmon
	},
	set_gproc_prop(oncall_ringing, inqueue_ringing, NewBase),
	{next_state, inqueue_ringing, {NewBase, NewInternal}};
...
handle_info({'DOWN', Ref, process, Pid, Info}, oncall, {BaseState,
	...
	#base_state{callrec = Call, callback = Callback, substate = Sub} = BaseState,
	 case priv_queue("default_queue", reprioritize_for_requeue(Call), get_init(BaseState#base_state.state_changes), false) of
		invalid ->
...
handle_info({cpx_conference_update, {Leg, State}}, StateName, {BaseState,
		#oncall_state{oncall_pid = {_Agent, Apid}} = Internal}) ->
	Call = BaseState#base_state.callrec,
	send_conference_update_to_channel(Apid, Leg, State, Call),
	{next_state, StateName, {BaseState, Internal}};

handle_info({gproc, registered, VqueueKey}, StateName, {BaseState, Internal}) ->
	RecoveryInfo = BaseState#base_state.recovery_info,
	VqueueKey0 = proplists:get_value(reach_vqueue_key, RecoveryInfo),
	OldPid = proplists:get_value(recovered_pid, RecoveryInfo),
	lager:info("Vqueue with key ~p/~p registered, sending ~p", [VqueueKey0, VqueueKey, {gen_media_recovery, OldPid, self()}]),
	case VqueueKey0 =:= VqueueKey of
		true ->
			catch gproc:send(VqueueKey, {gen_media_recovery, OldPid, self()});
		false ->
			ok
	end,
	{next_state, StateName, {BaseState, Internal}};
...
%%--------------------------------------------------------------------
%% Func: code_change(OldVsn, StateName, State, Extra) -> {ok, NewState}
%%--------------------------------------------------------------------
...
sync_call_cbk(Zt, {BaseSt, IntSt}, F, PreA, H) ->
	...
	lager:debug("Sync_call_cbk ~p:~p with args ~p", [M, F, A]),
...
handle_custom_return({voicemail, _}, wrapup, Reply, {BaseState, Internal}) ->
	case Reply of
		reply ->
			{reply, ok, wrapup, {BaseState, Internal}};
		noreply ->
			{next_state, wrapup, {BaseState, Internal}}
	end;
...
handle_custom_return({outgoing, {AgentName, AgentChannel}, Call, NewState}, StateName, Reply,
	...
	case {Response, Reply} of
		{ok, noreply} ->
			lager:info("Moving from ~p to oncall state, setting gproc", [StateName]),
			set_gproc_prop(StateName, oncall, BaseState),
...
handle_stop(Reason, StateName, BaseState, Internal) ->
	set_cpx_mon({BaseState, Internal}, delete),
	{Who, Reason2} = case Reason of
		{hangup, W} -> {W, normal};
		_ -> {undefined, Reason}
	end,
	agent_interact({hangup, Who}, StateName, {BaseState, Internal}),
	{Reason2, {BaseState, Internal}}.

enqueue(QueueN, Call, BaseSt) ->
	%% TODO if invalid
	Timestamp = get_init(BaseSt#base_state.state_changes),
	QNPid = case priv_queue(QueueN, Call, Timestamp, true) of
		{QueueN, Else} when is_pid(Else) ->
			{QueueN, Else};
		_ ->
			error
	end,
	...
	case QNPid of
		{QN, QPid} ->
			% {_Key, #queued_call{skills = QSkills}} =
			% 	reach_vqueue:get_call(QPid, self()),
			% lager:info("Qskills are ~p", [QSkills]),
			...
			ExpandedSkills = gen_media_util:expand_magic_skills(QueueN, Call, Queue#call_queue.skills),

			Call1 = Call#call{
				% skills = lists:usort(QSkills),
				priority = set_priority(Call, Queue),
				state_changes=StateChanges1,
				call_segment=Call#call.call_segment,
			...
			lager:info("Call1 in gen_media is ~p", [lager:pr(Call1, call)]),
			BaseSt1 = case BaseSt#base_state.init_callrec#call.queue of
				undefined ->
					BaseSt#base_state{callrec = Call1,
						init_callrec = Call#call{queue = QN},
						state_changes = StateChanges1};
					_ ->
						BaseSt#base_state{callrec = Call1,
						state_changes = StateChanges1}
...
ps_to_call(Ps, Base, Callback, StateChanges) when is_record(Base, call) ->
	...
	OriginalId = ?get(original_id, Ps, CallId),
	Type = ?get(type, Ps, Base#call.type),
	MediaType = ?get(media_type, Ps, Base#call.media_type),
	...
	CallSegment = ?get(call_segment, Ps, Base#call.call_segment),
...
priv_queue(Queue, Callrec, Timestamp, Failover) ->
	lager:info("In Priv Queue is ~p and Callrec is ~p and node is ~p", [Queue, Callrec, node()]),
	reach_vqueue_manager:add_to_queue(Queue, node(), self(), Callrec, Timestamp, Failover).

priv_voicemail(St) ->
	priv_voicemail(St, undefined).

priv_voicemail({BaseState, #inqueue_ringing_state{ring_mon = Rmon, ring_pid = {_, Rpid}} = Internal}, CookArgs) ->
	erlang:demonitor(Rmon),
	...
	priv_voicemail({BaseState, NewInternal}, CookArgs);

priv_voicemail({BaseState, #inqueue_state{queue_mon = Mon, queue_pid = {QNom, QPid},
		cook = Cook, cook_mon = CookMon}}, CookArgs) ->
	erlang:demonitor(Mon),
	lager:info("Going to voicemail, removing call ~p from queue ~p", [self(), QPid]),
	reach_vqueue:bgremove(QPid, self()),
	case Cook of
		_ when is_pid(Cook) ->
			erlang:demonitor(CookMon),
	...
	cdr:voicemail(BaseState#base_state.callrec, {QNom, CookArgs}),
	ok.
...
set_priority(Call, Queue) ->
	case Call#call.priority of
...
reprioritize_for_requeue(Call) ->
	NewPriority = Call#call.priority - 5,
...
unqueue(Qpid, Callpid) when is_pid(Qpid) ->
	reach_vqueue:remove_call(Qpid, Callpid),
	ok.
...
agent_interact({stop_ring, Reason}, oncall_ringing, #base_state{
	...
	lager:info("Moving from oncall_ringing to oncall state, setting gproc"),
	set_gproc_prop(oncall_ringing, oncall, BaseState),
...
agent_interact({hangup, Who}, oncall, #base_state{callrec = Callrec} =
	...
	cdr:hangup(Callrec, Who),
	catch erlang:demonitor(Mon),
	{wrapup, {BaseState, #wrapup_state{}}};

agent_interact(Msg, State, #base_state{callrec = Callrec} =
		BaseState, Internal, {_Mon, {_Agent, _Apid}}) when
		is_record(Callrec, call) ->
	lager:info("agent_interact got this msg ~p", [Msg]),
...
set_gproc_prop(OldMediaState, MediaState, FsmState) ->
	Prop = get_gproc_prop(MediaState, FsmState),
	gproc:set_value({p, g, cpx_media}, Prop),
...
send_conference_update_to_channel(AgentChannel, ConfEntry, State, Call) ->
	SourceModule = Call#call.source_module,
	LegId = ConfEntry#conference_leg.id,
	OriginalId = ConfEntry#conference_leg.original_id,
	{RequestType, RequestValue} = ConfEntry#conference_leg.request,
	Props = [
		{<<"leg_id">>, l2b(LegId)},
		{<<"original_id">>, l2b(OriginalId)},
		{<<"request_type">>, RequestType},
		{<<"request_value">>, l2b(RequestValue)},
		{<<"source_module">>, SourceModule},
		{<<"state">>, State}
	],
	AgentProp = case ConfEntry#conference_leg.agent of
		undefined ->
			{<<"agent">>, null};
		{Login, FName, LName} ->
			{<<"agent">>, [
				{<<"login">>, l2b(Login)},
				{<<"first_name">>, nob(FName)},
				{<<"last_name">>, nob(LName)}
			]}
	end,
	Update = {conference_update, Props ++ [AgentProp]},
	agent_channel:media_push(AgentChannel, Call, Update).

get_init(StateChanges) ->
	proplists:get_value(init, StateChanges).
...
update_skills_test_() ->
	...
	fun(_) ->
		...
		?assertEqual(#cpx_gen_media_prop{state=ringing, call=Call2}, gproc:get_value({p, g, cpx_media}))
		...
		?assertEqual(#cpx_gen_media_prop{state=ringing, call=Call3}, gproc:get_value({p, g, cpx_media}))
	end}]}.
