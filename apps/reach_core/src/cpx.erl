...
start(_Type, StartArgs) ->
	...
	Nodes = get_nodes(),
	lager:info("Reach nodes: ~p", [Nodes]),
	application:set_env(openacd, nodes, Nodes),
	ping_nodes(Nodes),
	gproc_dist:start_link(Nodes),
	reach_node_monitor:start_link(),
	reach_config_reloader:start_link(),
	...
	try cpx_supervisor:start_link(Nodes) of
		{ok, Pid} ->
			...
			lager:notice("Application OpenACD/Reach Core started sucessfully!", []),
			% to not block the shell.
			spawn(fun init_load_plugins/0),
			{ok, Pid}
	catch
		What:Why ->
			lager:error("Application OpenACD/Reach Core failed to start successfully! ~p:~p", [What, Why]),
			{What, Why}
	end.
...
notice(Msg) ->
	lager:notice("cpx received: ~p", [Msg]).

set_ring_timeout(Timeout) when is_integer(Timeout), Timeout > 0 ->
	lager:notice("Setting default ring timeout to ~p seconds", [Timeout]),
	application:set_env(reach_core, default_ring_timeout, Timeout),
	ok;
set_ring_timeout(Timeout) ->
	lager:warning("Invalid default value for ring timeout: ~p", [Timeout]),
	invalid_ring_timeout.
...
get_env(Key) ->
	application:get_env(reach_core, Key).

%% @doc @see gen_env/1
-spec(get_env/2 :: (Key :: any(), Default :: any()) -> {'ok', any()}).
get_env(Key, Default) ->
	case application:get_env(reach_core, Key) of
		undefined ->
			{ok, Default};
		Out ->
			Out
...
uptime(Fallback) ->
	...
	case Running of
		false ->
			stopped;
		true ->
			case {application:get_env(reach_core, uptime), Fallback} of
				{undefined, false} ->
...
print_call_list([]) ->
	io:format("End of Call List~n~n");
print_call_list([{_,QCall}|Others]) when is_record(QCall, queued_call) ->
	io:format("\t Call Id: ~p~n", [QCall#queued_call.id]),
	print_call_list(Others).
print_vqueue([]) ->
	ok;
print_vqueue([{_Qpid, {state, Name, Started, Queue, _Limbo, _Recipe, Skills, _Flag, Priority,
							AgingFactor, EligAg, FreeAg, _Ref, _Incarnation, _Notified, _Subscribed}}|Others]) ->
	io:format("Initial Queue: ~p~n", [Name]),
	io:format("Skills: ~p~n", [Skills]),
	io:format("Priority: ~p~n", [Priority * -1]),
	io:format("Aging Factor: ~p~n", [AgingFactor]),
	StartDate = ouc_time:unixts_to_ldatetime(Started div 1000000),
	DateString = datetime_to_datestring(StartDate),
	io:format("Started: ~p~n", [DateString]),
	Duration = ouc_time:now() - (Started div 1000000),
	{Day, Hour, Minute, Second} = seconds_to_duration(Duration),
	io:format("Duration: ~p Days ~p Hours ~p Minutes ~p Seconds~n", [Day, Hour, Minute, Second]),
	io:format("Eligible Agents: ~p~n", [EligAg]),
	io:format("Free Agents: ~p~n", [FreeAg]),
	io:format("Call List:~n"),
	CallList = gb_trees:to_list(Queue),
	print_call_list(CallList),
	print_vqueue(Others).
print_vqueues([]) ->
	Res = reach_vqueue_manager:get_all_info(),
	print_vqueue(Res).

graceful([]) ->
	shutdown_manager:shutdown(node()).

datetime_to_datestring({{Year, Month, Day}, {Hour, Minute, Second}}) ->
	Args = [Year, Month, Day, Hour, Minute, Second],
	S = io_lib:format("~B-~2.10.0B-~2.10.0BT~2.10.0B:~2.10.0B:~2.10.0BZ", Args),
	lists:flatten(S).

seconds_to_duration(Time) ->
	Days = Time div 86400,
	HrRem = Time rem 86400,
	Hours = HrRem div 3600,
	MinRem = HrRem rem 3600,
	Minutes = MinRem div 60,
	Seconds = MinRem rem 60,
...
get_nodes() ->
	NodesEnv = case application:get_env(reach_core, nodes) of
		{ok, Ns} -> Ns;
		_ -> []
	end,
