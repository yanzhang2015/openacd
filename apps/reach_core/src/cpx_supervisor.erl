...
start_link(Nodes) ->
	...
	VQueueManagerSpec = {reach_vqueue_manager, {reach_vqueue_manager, start_link, [Nodes]}, permanent, 20000, worker, [?MODULE]},
	ShutdownManagerSpec = {shutdown_manager, {shutdown_manager, start_link, [Nodes]}, permanent, 20000, worker, [?MODULE]},
	%CallSegmentReportSpec = {ouc_call_segment_report_job, {ouc_call_segment_report_job, start_link, [Nodes]}, permanent, 20000, worker, [?MODULE]},
	...
	{ok, _} = supervisor:start_child(routing_sup, ShutdownManagerSpec),
	%{ok, _} = supervisor:start_child(routing_sup, CallSegmentReportSpec),
	{ok, _} = supervisor:start_child(routing_sup, QueueManagerSpec),
	{ok, _} = supervisor:start_child(routing_sup, VQueueManagerSpec),
...
restart(routing_sup, Nodes) ->
	...
	supervisor:start_child(routing_sup, ShutdownManagerSpec),
	%supervisor:start_child(routing_sup, CallSegmentReportSpec),
	supervisor:start_child(routing_sup, QueueManagerSpec),
	supervisor:start_child(routing_sup, VQueueManagerSpec),
...
set_value(Key, Value) ->
	...
	{ok, Locked} = application:get_env(reach_core, locked_env),
