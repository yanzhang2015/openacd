...
cdrinit(Call) ->
	...
	lists:foreach(fun(M) -> add_cdr_mod(M, Call) end, lists:flatten(Ms)).

loginit(Call) ->
	{ok, Ms} = cpx_hooks:trigger_hooks(get_log_mods, [], all),
	lists:foreach(fun(M) -> add_log_mod(M, Call) end, lists:flatten(Ms)).
...
add_skills(Call, Data) ->
	event({add_skills, Call, util:now(), Data}).

remove_skills(Call, Data) ->
	event({remove_skills, Call, util:now(), Data}).

announce(Call, Data) ->
	event({announce, Call, util:now(), Data}).
...
agent_transfer(Call, Data) ->
	event({agent_transfer, Call, util:now(), Data}).

%% @doc Notify cdr handler that `#call{} Call' is being offered by `string() Offerer'
%% to `string() Recipient'.
-spec(queue_transfer/2 :: (Call :: #call{}, Queue :: string()) -> 'ok').
queue_transfer(Call, Data) ->
	event({queue_transfer, Call, util:now(), Data}).

outband_transfer(Call, Data) ->
	event({outband_transfer, Call, util:now(), Data}).

agent_conference(Call, Data) ->
	event({agent_conference, Call, util:now(), Data}).

queue_conference(Call, Data) ->
	event({queue_conference, Call, util:now(), Data}).

%% @doc Notify the cdr handler the `#call{} Call' is being sent to voicemail
%% from `string() Queue'.
-spec(voicemail/2 :: (Call :: #call{}, Queue :: pid() | string()) -> 'ok').
voicemail(Call, {Qpid, Args}) when is_pid(Qpid) ->
...
init([Call]) ->
	...
	Nodes = case application:get_env(reach_core, nodes) of
		undefined ->
			[node()];
		{ok, List} ->
			List
...
handle_event({Transaction, #call{id = Callid} = Call, Time, Data}, #state{id = Callid, limbo_wrapup_count = Limbocount} = State) ->
	Ended = case lists:member(Transaction, [hangup, agent_transfer, voicemail, endwrapup, queue_transfer, outband_transfer, ringout]) of
		true ->
			Time;
		false ->
...
find_untermed(queue_transfer, _, _) ->
	...
	[];
find_untermed(outband_transfer, _, _) ->
	[];
find_untermed(agent_conference, _, _) ->
	[];
find_untermed(queue_conference, _, _) ->
	[];
