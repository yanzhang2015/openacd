...
-define(STORE,
	{ok, _Store} = application:get_env(reach_core, call_queue_config_storage),
	_Store).

-spec(start/0 :: () -> any()).
start() ->
	Store = case application:get_env(reach_core, call_queue_config_storage) of
		{ok, St} ->
			St;
		_ ->
			St = ?DEFAULT_STORAGE,
			application:set_env(reach_core, call_queue_config_storage, St),
			St
	end,
	Store:start().
...
get_default_queue() ->
	{ok, Store} = application:get_env(reach_core, call_queue_config_storage),
...
passthrough_test_() ->
	{foreach, fun() ->
		...
		application:set_env(reach_core, call_queue_config_storage, mock_cqueue)
	end, fun(_) ->
		application:unset_env(reach_core, call_queue_config_storage),
		meck:unload()
	end, [
