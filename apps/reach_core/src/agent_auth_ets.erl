...
setup(Opts) ->
	...
	LicenseLimit = proplists:get_value(license_limit, Opts, none),
	load_license_limit(LicenseLimit),

	Permissions = proplists:get_value(permissions, Opts, none),
...
get_license_seats() ->
  case ets:lookup(cpx_settings, license_limit) of
    [Res] ->
      {ok, Res};
    _ ->
      none
...
load_license_limit(LicenseLimit) ->
  ets:delete_all_objects(cpx_settings),
  ets:insert(cpx_settings, {license_limit, LicenseLimit}).

-spec has_permission(PermProfile :: atom(), PermName :: tuple() | binary()) -> {ok, true} | {ok, false} | {error, any}.
has_permission(PermProfile, PermKey) ->
  case ets:lookup(cpx_permissions, atom_to_binary(PermProfile, utf8)) of
    [] -> {ok, false};
    PropList ->
      {Key, Value} = case PermKey of
        {K, V} -> {K, V};
        Other -> {Other, undefined}
      end,

      Res = proplists:get_value(Key, PropList),
      case is_boolean(Res) of
        true ->
          {ok, Res};
        _ when is_list(Res)->
          {ok, lists:member(Value, Res)}
      end
  end.

-spec load_permissions(Permissions :: list(tuple())) -> true | {error, any()}.
load_permissions(Permissions) ->
  ets:delete_all_objects(cpx_permissions),
  ets:insert(cpx_permissions, Permissions).
...
loop0(P) ->
	try
		...
		ets:new(cpx_settings, [{keypos, license_limit}, named_table, public]),
		ets:new(cpx_permissions, [{keypos, 1}, named_table, public]),
