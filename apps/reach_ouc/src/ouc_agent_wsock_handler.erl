%% Copyright (c) 2012-2015 eZuce, Inc. All rights reserved.
...
%% Old cpx_agent_wsock_handler moved to reach_ouc

-module(ouc_agent_wsock_handler).
-author("jvliwanag").
...
websocket_init(_TransportName, Req, Opts) ->
	...
	send_timeout_check(),
    lager:debug("websocket request ~p ",[Req]),
    case ouc_session:auth(Req) of
	%case cpx_hooks:trigger_hooks(wsock_auth, [Req]) of
		{ok, {Login, AOpts, Req2}} ->
			case cpx_agent_connection:start(Login, AOpts) of
				{ok, Agent, Conn} ->
        ...
        {error, Err} ->
            lager:warning("Trigger hook returned the err: ~p ",[Err]),
...
websocket_info(timeout_check, Req, State) ->
	...
	case Diff > State#state.timeout_ms of
		true ->
			lager:info("Shutting down wsock connection for ~p due to timeout", [self()]),
			{shutdown, Req, State};
		_ ->
			send_timeout_check(),
			{ok, Req, State}
...
websocket_info(M, Req, State) ->
	try handle_ws_info(State#state.info_handlers, State#state.conn, M) of
		...
		_ ->
            lager:warning("Received unhandled info ~p in state ~p; request ~p", [M, State, Req]),
			{ok, Req, State}
	catch
		T:Err ->
			Trace = erlang:get_stacktrace(),
...
%% @todo read wsock port from sys.config
init_response(Username, SLevel) ->
	...
	WsockUrls = case application:get_env(reach_core, nodes) of
		{ok, Nodes} ->
			{WsUrls, WssUrls} = lists:foldl(fun(N, {Ws, Wss}) ->
				NStr = atom_to_list(N),
				[_, Domain] = string:tokens(NStr, "@"),
				Url1 = list_to_binary("ws://" ++ Domain ++ ":8937/wsock"),
				Url2 = list_to_binary("wss://" ++ Domain ++ ":8936/wsock"),
				{[Url1|Ws], [Url2|Wss]}
			end, {[], []}, Nodes),
			{[{wss, WssUrls}, {ws, WsUrls}]};
		_ ->
			{[{wss, []}, {ws, []}]}
	end,
	Resp = {struct, [
