...
-record(spx_fwconfig, {cnodes, dialstring="", custom_headers=""}).
...
jprop_to_config([{<<"active">>, false} | _], _) ->
    {ok, none};
jprop_to_config([{<<"nds">>, {array, CnodesBin}} | T], Acc) ->
    Cnodes = [binary_to_atom(Cnode, utf8) || Cnode <- CnodesBin],
	jprop_to_config(T, Acc#spx_fwconfig{cnodes = Cnodes});
jprop_to_config([{<<"dlst">>, DialString} | T], Acc) ->
	jprop_to_config(T, Acc#spx_fwconfig{dialstring = binary_to_list(DialString)});
jprop_to_config([{<<"custheaders">>, CustomHeaders} | T], Acc) ->
  jprop_to_config(T, Acc#spx_fwconfig{custom_headers = CustomHeaders});
	...
	cpx_supervisor:update_conf(freeswitch_media_manager,
		#cpx_conf{
			...
			module_name = freeswitch_media_manager,
				...
				start_args = [
					Conf#spx_fwconfig.cnodes,
					[{h323,[]}, {iax2,[]}, {sip,[]},
					 {dialstring, Conf#spx_fwconfig.dialstring},
					 {custom_headers, Conf#spx_fwconfig.custom_headers}]],
