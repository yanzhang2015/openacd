...
get_license_seats(Props) ->
  {ok, get_int(<<"cru">>, Props)}.
...
%% Security Level
build_agent([{<<"permp">>, Sec} | Tail], Rec) ->
	build_agent(Tail, Rec#agent_auth{security_level = binary_to_atom(Sec, utf8)});
...
build_agent([{<<"cnt">>, SipURI}|T], Acc) ->
	...
	ConfEndpoint = {freeswitch_conference, EndpointOpts},

	OldEndpoints = Acc#agent_auth.endpoints,
	build_agent(T, Acc#agent_auth{endpoints = [FwEndpoint, VmEndpoint, ConfEndpoint|OldEndpoints]});
...
build_queue([{<<"aging">>, [{<<"int">>,Int},{<<"remainder">>,Rem}]}|T], Acc) ->
	build_queue(T, Acc#call_queue{aging_factor=Int + Rem / 10});
build_queue([_|T], Acc) ->
	build_queue(T, Acc).
...
read_recipe_action(<<"set_priority">>, P) ->
	case proplists:get_value(<<"actionValue">>, P) of
		...
		Bin when is_binary(Bin) ->
			{ok, {set_priority, erlang:trunc(list_to_integer(binary_to_list(Bin)))}};
		_ ->
			{error, invalid_vlu}
	end;
...
read_recipe_action(<<"run_ivr">>, P) ->
	case proplists:get_value(<<"actionValue">>, P) of
		Bin when is_binary(Bin) ->
			{ok, {run_ivr, [{script, binary_to_list(Bin)}]}};
		_ ->
			{error, invalid_vlu}
	end.
...
build_client([{<<"recCalls">>, Enabled}|T], Acc)
				when is_boolean(Enabled) ->
	Opts = Acc#client.options,
	build_client(T, Acc#client{options =
		[{recording_enabled, Enabled}|Opts]});
build_client([{<<"callsDir">>, Path}|T], Acc)
				when is_binary(Path) ->
	Opts = Acc#client.options,
	build_client(T, Acc#client{options =
		[{recording_dir, binary_to_list(Path)}|Opts]});
build_client([{<<"retainCalls">>, Count}|T], Acc)
				when is_number(Count) andalso Count > 0 ->
	Opts = Acc#client.options,
	build_client(T, Acc#client{options =
		[{recording_count, trunc(Count)}|Opts]});
build_client([{<<"agCallerID">>, AgCallerID}|T], Acc)
        when is_boolean(AgCallerID) ->
  Opts = Acc#client.options,
  build_client(T, Acc#client{options =
    [{use_agent_callerid, AgCallerID}|Opts]});
build_client([{<<"inbCallerID">>, InboundCallerId}|T], Acc)
        when is_binary(InboundCallerId) ->
  Opts = Acc#client.options,
  build_client(T, Acc#client{options =
    [{inbound_caller_id, InboundCallerId}|Opts]});
build_client([{<<"outbCallerID">>, OutboundCallerId}|T], Acc)
        when is_binary(OutboundCallerId) ->
  Opts = Acc#client.options,
  build_client(T, Acc#client{options =
    [{outbound_caller_id, OutboundCallerId}|Opts]});
